...
Use with `frontend/architecture-and-workflow` when changing dashboard data flows, project/file summaries, viewer initialization, shared/view-only bundles, or viewer route/query behavior. For generic routing/websocket behavior, read `frontend/routing-app-shell-subtleties`.
