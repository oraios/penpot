...
* **GitHub MCP Server** provides tools for interacting with GitHub (issue, PRs, etc.)
...
this can be achieved by launching the browser with the `--remote-debugging-port` flag, e.g.:
...
Note: For security reasons, you should not enable remote debugging on a browser instance
that you use for regular browsing activities.
We recommend to use a separate browser for Penpot development.
...
When logged in, navigate to your account settings, click on "Integrations" in the sidebar, and enable the "MCP Server" toggle.
...
For the GitHub MCP server, you need to provide a personal access token (PAT) with appropriate permissions:
